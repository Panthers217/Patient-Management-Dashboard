...
const createRouter = () => {
  ...
  router.get('/:mrn', async (req, res) => {
    ...
    // fetch encounters separately to avoid generated type dependency during dev
  ...
  router.post('/:mrn/encounters', async (req, res) => {
    const { mrn } = req.params
  ...
  // update an encounter
  router.put('/:mrn/encounters/:id', async (req, res) => {
